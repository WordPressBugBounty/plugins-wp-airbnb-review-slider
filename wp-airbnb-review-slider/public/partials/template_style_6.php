<?php
/**
 * Template Style 6 — card layout with avatar header row (Airbnb).
 *
...
for ( $x = 0; $x < count( $rowarray ); $x++ ) {
	...
	foreach ( $rowarray[ $x ] as $review ) {
		...
		if ( isset( $template_misc_array['avataropt'] ) && $template_misc_array['avataropt'] === 'init' ) {
			...
			$userpic = $imgs_url . 'airbnb_mystery_man.png';
		} else {
		...
		if ( isset( $review->from_url ) && ! empty( $review->from_url ) ) {
			...
			$burl    = isset( $options['airbnb_business_url'] ) ? $options['airbnb_business_url'] : 'https://www.airbnb.com';
			if ( $burl === '' ) {
		...
		if ( isset( $template_misc_array['verified'] ) && $template_misc_array['verified'] === 'yes1' ) {
			$verifiedhtml = '<span class="verifiedloc1 wprevpro_verified_svg wprevtooltip" data-wprevtooltip="' . esc_attr__( 'Verified on Airbnb', 'wp-airbnb-review-slider' ) . '"><span class="svgicons svg-wprsp-verified"></span></span>';
		}

		// site icon/logo (honors "Show Icon" setting: no / yes / lin)
		$logo_img  = '<img src="' . esc_url( $imgs_url . 'airbnb_outline.png' ) . '" alt="' . esc_attr__( 'Airbnb logo', 'wp-airbnb-review-slider' ) . '" class="wprevpro_t6_site_logo siteicon sitetype_Airbnb">';
		$site_logo = '';
