 ...
 * @subpackage WP_Airbnb_Review_Slider/public/partials
 */
...
if ( ! class_exists( 'Template_Functions' ) ) {
	...
	class Template_Functions {
		...
		/**
		 * Get media HTML for a review. Defaults to showing media unless explicitly off.
		 *
		...
		public function wprevpro_get_media( $review, $template_misc_array ) {
			...
			$mediathumburlsjson = isset( $review->mediathumburlsarrayjson ) ? stripslashes( $review->mediathumburlsarrayjson ) : '';
			$mediathumburlsarr  = $mediathumburlsjson !== '' ? json_decode( $mediathumburlsjson, true ) : array();
			...
			foreach ( $mediaurlsarray as $urlvalue ) {
				if ( $urlvalue !== '' ) {
					...
					if ( is_array( $mediathumburlsarr ) && isset( $mediathumburlsarr[ $n ] ) && $mediathumburlsarr[ $n ] !== '' ) {
						$thumburl = esc_url( $mediathumburlsarr[ $n ] );
					} else {
						$thumburl = $urlvalue;
					}
					$tempclass = ( stripos( $urlvalue, 'youtu' ) === false ) ? 'notyoutu' : 'youtu';
					$media    .= '<a class="wprev_media_img_a ' . $tempclass . '" href="' . $urlvalue . '" data-lity><img src="' . $thumburl . '" class="wprev_media_img" alt="media thumbnail ' . $n . '"></a>';
				}
				$n++;
			}
			$media .= '</div>';
		...
		public function wprevpro_get_reviewername( $review, $template_misc_array ) {
			...
			if ( $template_misc_array['lastnameformat'] === 'hide' ) {
				...
				if ( isset( $words[1] ) && $words[1] !== '' ) {
					$tempreviewername .= ' ' . strtoupper( mb_substr( $words[1], 0, 1 ) ) . '.';
				}
			} else {
				$templast         = isset( $words[1] ) ? $words[1] : '';
				$tempreviewername = trim( $firstname . ' ' . $templast );
			}
