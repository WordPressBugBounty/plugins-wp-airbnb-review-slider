(function( $ ) {
	...
	$(function(){
		...
		var sampltext = 'This is a sample review. Hands down the best stay we have had! Awesome host and location. We will gladly recommend this place to all our friends!';
		var verified1 = '<span class="verifiedloc1 wprevpro_verified_svg wprevtooltip" data-wprevtooltip="Verified on Airbnb"><span class="svgicons svg-wprsp-verified"></span></span>';
		var iconhref = pluginsUrl + '/public/partials/imgs/airbnb_outline.png';
		var iconhtml = '<img id="wprev_showicon" src="'+iconhref+'" alt="Airbnb" class="wprevpro_t1_site_logo siteicon">';
		var iconhtmlt6 = '<img id="wprev_showicon" src="'+iconhref+'" alt="Airbnb" class="wprevpro_t6_site_logo siteicon">';

		var imagehref = pluginsUrl + '/admin/partials/sample_avatar.jpg';
		var imagehrefmystery = pluginsUrl + '/public/partials/imgs/airbnb_mystery_man.png';
		...
		var wpairbnbTooltipRoots = "#wprevpro_template_preview, #wpairbnb_preview_outer";
		$( wpairbnbTooltipRoots ).on('mouseenter touchstart', '.wprevtooltip', function(e) {
			var titleText = $(this).attr('data-wprevtooltip');
			$(this).data('tiptext', titleText).removeAttr('data-wprevtooltip');
			$('<p class="wprevpro_tooltip"></p>').text(titleText).appendTo('body').css('top', (e.pageY - 15) + 'px').css('left', (e.pageX + 10) + 'px').fadeIn('slow');
		});
		$( wpairbnbTooltipRoots ).on('mouseleave touchend', '.wprevtooltip', function(e) {
			$(this).attr('data-wprevtooltip', $(this).data('tiptext'));
			$('.wprevpro_tooltip').remove();
		});
		$( wpairbnbTooltipRoots ).on('mousemove', '.wprevtooltip', function(e) {
		...
		function changepreviewhtml(){
			...
			if($( "#wpairbnb_template_css" ).val()!=""){
				prestyle += '<style>'+$( "#wpairbnb_template_css" ).val()+'</style>';
			}
		...
		$( ".wpairbnb_helpicon_p" ).click(function() {
			$(this).closest('tr').find('p.description').toggleClass('wpairbnb_desc_visible');
		});
		...
		$( ".wpairbnb_displayshortcode" ).click(function() {
			...
			if(ttype=="widget"){
				...
				openpopup("How to Display", '<p>Enter this shortcode on a post, page, or text widget: </br></br>[wpairbnb_usetemplate tid="'+tid+'"]</p>', '');
			}
		});
		...
		function initpreviewsliders(){
			$( "#wpairbnb_preview_outer .wprev-slider" ).each(function(){
				createaslider(this, 'shortcode');
			});
			$( "#wpairbnb_preview_outer .wprev-slider-widget" ).each(function(){
		...
		function missingimgcheck(){
			$('#wpairbnb_preview_outer img').each(function () {
				var img = this;
				var $img = $(this);
		...
		function showtemplatepreview(){
			...
			$( "#wpairbnb_preview_outermost" ).show();
			$( "#loadingpreview" ).addClass('is-active');
			var senddata = {
				action: 'wpairbnb_get_preview',
			...
			jQuery.post(ajaxurl, senddata, function(response){
				...
				if(response){
					try {
						...
						$( "#wpairbnb_preview_outer" ).html(result.templatehtml);
						initpreviewsliders();
					} catch(e){
		...
		$( "#wpairbnb_addnewtemplate_update" ).click(function(e){
			e.preventDefault();
				...
				if(response){
					try {
						...
						if(saveresult.ack=='success'){
							...
							$( "#wpairbnb_preview_outer" ).html(saveresult.templatehtml);
							initpreviewsliders();
						} else {
		...
		$( "#wpairbnb_preview_outer" ).on( "click", ".wprs_rd_more", function(){
			$(this).hide();
		...
		$( "#wpairbnb_t_blocation" ).change(function() {
			hideshowbadgeoptions(false);
		});
		function hideshowbadgeoptions(instant){
			if($( "#wpairbnb_t_blocation" ).val()==""){
		...
		function setbadgetitle(){
			var $opt = $( "#wpairbnb_t_filtersource option:selected" );
			if(!$opt.length || !$opt.val()){ return; }
			var sname = $opt.text();
			var surl = $opt.attr('data-fromurl') || '';
			if(sname && $( "#wpairbnb_t_bname" ).val()==""){
				$( "#wpairbnb_t_bname" ).val(sname);
			}
			if(surl){
				if($( "#wpairbnb_t_bnameurl" ).val()==""){
					$( "#wpairbnb_t_bnameurl" ).val(surl);
				}
				if($( "#wpairbnb_t_bbtnurl" ).val()==""){
					$( "#wpairbnb_t_bbtnurl" ).val(surl);
				}
			}
		}
		$( "#wpairbnb_t_filtersource" ).change(function() {
			setbadgetitle();
		});
		setbadgetitle();

		//------------Badge business image uploader (wp.media)------------
		var wpairbnbBadgeMediaFrame;
		$('#upload_licon_button').on('click', function(e) {
			e.preventDefault();
			if (typeof wp === 'undefined' || !wp.media) {
				return false;
			}
			if (wpairbnbBadgeMediaFrame) {
				wpairbnbBadgeMediaFrame.open();
				return false;
			}
			wpairbnbBadgeMediaFrame = wp.media({
				title: 'Upload Icon',
				button: { text: 'Use this image' },
				library: { type: 'image' },
				multiple: false
			});
			wpairbnbBadgeMediaFrame.on('select', function() {
				var attachment = wpairbnbBadgeMediaFrame.state().get('selection').first().toJSON();
				if (attachment && attachment.url) {
					$('#wpairbnb_t_bimgurl').val(attachment.url);
				}
			});
			wpairbnbBadgeMediaFrame.open();
			return false;
